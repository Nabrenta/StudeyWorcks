...
public:
   ...
   void Menu(void);//Меню читального залу
   void AssesOfWiFi(bool);//встановити модифікатор доступу до мережі WiFi
   void setScanPrise(int);//повернути вартість скану
   void setCopyPrise(int);//повернути вартість копії
   void GiveServises(void);//Виконати сканування або друк
   void GiveWiFi(void);//Надати WiFi
   static void pay();//Оплата
protected:
private:
